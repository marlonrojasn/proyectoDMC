...
if Módulos == "Home":
    ...
    if saldo_final > 0:
        ...
        st.success(
            f"Flujo de caja a favor: {saldo_final}"
        )

    elif saldo_final < 0:

        st.error(
            f"Flujo de caja en contra: {saldo_final}"
        )

    else:

        st.info(
            "El flujo de caja está en equilibrio"
        )
